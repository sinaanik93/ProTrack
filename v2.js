...
osRenderHome = function(root) {
  if (OS.user.role === "player") {
    const p=osVisiblePlayers()[0];if(!p){root.innerHTML=`<div class="os-card pt2-empty">${pt2Fa("No player profile is linked to this account.","پروفایل بازیکنی به این حساب متصل نیست.")}</div>`;return;}
...
document.addEventListener("click",event=>{
  ...
  const handled=["logout","v2-add-coach","v2-add-registration","v2-add-booking","v2-add-program","v2-add-story","v2-edit-player","v2-archive-player","v2-restore-player","v2-delete-player","v2-advance-registration","v2-enroll-registration","v2-edit-academy","v2-add-competition"];
